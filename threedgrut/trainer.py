...
from threedgrut.utils.misc import check_step_condition, create_summary_writer, jet_map, viridis_map
...
class Trainer3DGRUT:
    ...
    def get_metrics(
        ...
        if is_compute_validation_metrics:
            ...
            if iteration in self.conf.writer.log_image_views:
                ...
                metrics["img_pred_dist"] = viridis_map(outputs["pred_dist"][-1], 20)
                metrics["img_pred_normals"] = ((outputs["pred_normals"][-1].clip(-1.0, 1.0) + 1.0) * 0.5).detach().cpu()
    ...
    def log_validation_pass(self, metrics: dict[str, Any]) -> None:
        ...
        if "img_pred_normals" in metrics:
            writer.add_images(
                "image/normals/val",
                torch.stack(metrics["img_pred_normals"]),
                global_step,
                dataformats="NHWC",
            )
