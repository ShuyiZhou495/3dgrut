...
SplatRaster::trace(uint32_t frameNumber, int numActiveFeatures,
    ...
    torch::Tensor rayGGGSTransmittanceDebug = torch::zeros({height, width, 4}, opts);
    torch::Tensor rayGGGSSearchDebug = torch::zeros({height, width, 4}, opts);
    ...
    ErrorCode status = m_renderer->renderForward(
        ...
        reinterpret_cast<tcnn::vec4*>(voidDataPtr(rayGGGSTransmittanceDebug)),
        reinterpret_cast<tcnn::vec4*>(voidDataPtr(rayGGGSSearchDebug)),
    ...
    return std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>(
        rayRadianceDensity, rayHitDistance, rayHitCount, rayNormal, rayGGGSDebug, rayGGGSTransmittanceDebug, rayGGGSSearchDebug, particleVisibility);
}
